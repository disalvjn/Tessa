...
module Runtime = 
    ...
    let handleMessage (message: EvaluatorMessage) (runtime: Runtime) : Runtime = 
        ...
        | NoteTessellation (L.Tessellation(cell, effects)) -> 
            let withReversedEffects = L.Tessellation(cell, List.rev effects)
            {runtime with tessellations = withReversedEffects :: runtime.tessellations}
    ...
    let mergeDown topFrameRuntime continuationRuntime =
        {continuationRuntime with 
            ...
            tessellations = topFrameRuntime.tessellations @ continuationRuntime.tessellations;}